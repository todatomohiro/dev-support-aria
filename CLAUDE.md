...
> Live2D + LLM（Gemini/Claude）+ Amazon Polly TTS を活用したクロスプラットフォーム対応のアシスタントアプリ
...
pnpm test             # 全テスト実行（343テスト / 20ファイル）
...
| `ttsService` | `TtsServiceImpl` | Amazon Polly 音声合成・再生（Tomoko/neural 固定） |
...
| Lambda × 5 | Node.js 22.x / ARM_64（settings, messages, tts/synthesize） |

API エンドポイント: `/settings`（GET/PUT）, `/messages`（GET/POST）, `/tts/synthesize`（POST）
