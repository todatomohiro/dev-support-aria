...
■ 関連人物の抽出:
ユーザー自身だけでなく、会話に登場する重要な他者（家族、恋人、友人、ペットなど）に関する事実（名前、関係性、好み、状況）も積極的に抽出してください。
...
- 完全に重複する内容は出力しない
- ただし、記録済みの内容から状況が変化・進展している場合（例: 「転職活動中」→「株式会社○○に就職」）は、最新の事実として抽出すること
...
リストは時系列順（末尾が最新）で提供されます。

ルール:
1. 関連情報の統合: 意味的に関連する複数の事実は、1つの論理的な文に統合する
2. 表現揺れの統合: 同じ内容を異なる表現で記述したもの（例: "東京在住" と "東京に住んでいる"）は、より具体的で簡潔な1つの表現にマージする
3. 矛盾・時間的変化の解決: 時間経過によって矛盾する事実がある場合は、最新（リスト末尾側）の情報を正として古い情報を破棄・更新する
4. 情報の保持: 家族構成、健康状態、仕事、重要な趣味などの「ユーザーのコアアイデンティティ」に関わる情報は絶対に削除しない
5. 推測の排除: 提供されたリストに含まれない情報を捏造・推測して追加しない
6. 文字数制限: 統合後の各項目は最大50文字以内
7. 件数目標: 元の件数の60〜70%程度に圧縮する
8. 出力形式: 統合された事実リストのみを、1行1事実で出力する（番号・記号は不要）

統合の例:
- 「犬を飼っている」+「愛犬のポチは柴犬」→「柴犬のポチを飼っている」
- 「東京で一人暮らし」+「来月から大阪に転勤」→「来月から大阪に転勤予定」
- 「Pythonが好き」+「Reactを勉強中」+「AWSを使っている」→「Python/React/AWSでWeb開発」`
...
export const handler: Handler<ExtractFactsEvent, void> = async (event) => {
  ...
  // 6. 既存とマージ（完全一致の重複を排除）
  let mergedFacts = [...new Set([...existingFacts, ...extracted.facts])]
  let mergedPreferences = [...new Set([...existingPreferences, ...extracted.preferences])]
